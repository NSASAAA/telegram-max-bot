...
- Поддерживает интерактивную навигацию inline-кнопками (`Последние`, `Топ`, `Случайная`, `Рубрики`, `О боте`).
- При навигации старый экран удаляется и заменяется новым, чтобы не накапливать длинную ленту сообщений.
- Показывает рубрики статей командами `/topics` и `/topic_rs`, `/topic_family` и т.п.
...
- `/topic_rs` - жизнь рядом с РС
- `/topic_family` - брак и семья
- `/topic_children` - дети и воспитание
- `/topic_women` - женская роль и границы
- `/topic_psychology` - психология
- `/topic_faith` - вера без насилия
- `/topic_daily` - быт и повседневность
- `/topic_culture` - Москва и культура

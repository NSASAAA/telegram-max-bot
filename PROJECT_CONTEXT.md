...
1. Поддерживать фоновый автоимпорт через `CHECK_INTERVAL_SECONDS` (текущий рабочий режим).
2. Улучшить модель базы для будущей RSS-синхронизации.
3. Позже добавить автопубликацию новых статей в Telegram-канал.
4. После получения реальной RSS-ленты заказчика проверить, отдает ли она полный текст.
5. После накопления корпуса статей сделать анализ тем и сценариев общения.

...
1. Проверить на VPS работу `/check` и фонового автоимпорта по `CHECK_INTERVAL_SECONDS`.
2. Выбрать итоговый режим автоимпорта: оставить фоновый (`CHECK_INTERVAL_SECONDS`) или перенести на cron/job на VPS.
3. Улучшить модель базы для будущей RSS-синхронизации.
4. Позже добавить автопубликацию новых статей в Telegram-канал.
5. После получения реальной RSS-ленты заказчика проверить, отдает ли она полный текст.
6. После накопления корпуса статей сделать анализ тем и сценариев общения.

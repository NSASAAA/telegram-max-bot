...
Текущий импорт запускается вручную:
...
Будущая задача: автоматизировать запуск RSS-импорта, чтобы новые статьи попадали в базу без ручной команды. Для MVP предпочтительный вариант — cron на VPS, позже можно заменить на отдельный job/worker или встроенную фоновую задачу.
...
1. Автоматизировать RSS-импорт по расписанию, сначала через cron на VPS.
2. Обновить README под текущее состояние `/articles`, RSS, ручной импорт и volume `data/`.
3. Улучшить модель базы для будущей RSS-синхронизации.
4. Переделать `save_posts()` с `INSERT OR IGNORE` на `created / updated / unchanged`.
5. Добавить ручную команду проверки RSS, например `/check`.
6. Позже добавить автопубликацию новых статей в Telegram-канал.
7. После получения реальной RSS-ленты заказчика проверить, отдает ли она полный текст.
8. После накопления корпуса статей сделать анализ тем и сценариев общения.
